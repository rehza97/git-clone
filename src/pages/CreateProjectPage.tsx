...
import { Check, ArrowLeft, ArrowRight, Rocket, GraduationCap, User } from "lucide-react"
    ...
    <div className="flex min-h-screen flex-col font-sans text-foreground antialiased bg-background">
      <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col justify-center px-6 py-12 lg:px-8">
        ...
        <div className="relative mb-12 w-full max-w-3xl mx-auto">
          ...
          <div
            className="absolute top-1/2 left-0 h-0.5 -translate-y-1/2 -z-10 bg-primary transition-all duration-500"
            style={{ width: `${progressPercent}%` }}
          />
          <div className="flex w-full justify-between">
            <button type="button" onClick={() => setStep(1)} className="flex flex-col items-center gap-2 cursor-pointer group">
              <div className={`flex size-10 items-center justify-center rounded-full font-bold ring-4 ring-background shadow-sm ${step > 1 ? "bg-primary text-primary-foreground" : step === 1 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                {step > 1 ? <Check className="h-5 w-5" /> : "1"}
              </div>
              <span className={`text-sm font-semibold ${step >= 1 ? "text-primary" : "text-muted-foreground"}`}>{t("createProject.stepUniversity")}</span>
            </button>
            <button type="button" onClick={() => step >= 2 && setStep(2)} className="flex flex-col items-center gap-2 cursor-pointer group">
              <div className={`relative z-10 flex size-10 items-center justify-center rounded-full font-bold ring-4 ring-background shadow-sm ${step > 2 ? "bg-primary text-primary-foreground" : step === 2 ? "border-2 border-primary bg-card text-primary" : "bg-muted text-muted-foreground opacity-50"}`}>
                {step === 2 && <span className="absolute inset-0 rounded-full bg-primary/20 animate-ping" />}
            ...
            <button type="button" onClick={() => step >= 3 && setStep(3)} className={`flex flex-col items-center gap-2 cursor-pointer group ${step >= 3 ? "" : "opacity-50"}`}>
              <div className={`flex size-10 items-center justify-center rounded-full font-bold ring-4 ring-background shadow-sm ${step === 3 ? "border-2 border-primary bg-card text-primary" : "bg-muted text-muted-foreground"}`}>
                3
              </div>
            ...
            {step === 1 && (
              <div className="animate-in fade-in duration-200">
                ...
                <div className="flex flex-wrap gap-4">
                  <Button onClick={() => { setStep(2) }} className="rounded-lg bg-primary px-6 py-3 font-semibold text-primary-foreground shadow-lg shadow-primary/25 hover:bg-primary/90">
            ...
            {step === 2 && (
              <div className="animate-in fade-in duration-200">
                ...
                <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-3">
                  {PROJECT_TYPES.map(({ value, icon: Icon, titleKey, descKey, recommended }) => (
                    <label key={value} className="group relative cursor-pointer">
                      ...
                      <div className="h-full rounded-xl border-2 border-border bg-muted/60 p-6 transition-all duration-200 hover:border-primary hover:bg-card peer-checked:border-primary peer-checked:bg-primary/10">
                        ...
                        <div className={`flex items-center gap-2 text-xs font-medium text-primary transition-opacity ${projectType === value ? "opacity-100" : "opacity-0"}`}>
                          <Check className="h-4 w-4" />
                          {recommended ? t("createProject.recommended") : t("createProject.selected")}
                        </div>
                      </div>
                      <div className={`absolute right-4 top-4 text-primary transition-opacity ${projectType === value ? "opacity-100" : "opacity-0"}`}>
                ...
                <div className="flex items-center justify-between border-t border-border pt-8">
                  ...
                  <Button onClick={() => setStep(3)} className="flex items-center gap-2 rounded-lg bg-primary px-8 py-3 font-semibold text-primary-foreground shadow-lg shadow-primary/25 transition-all hover:scale-[1.02] hover:bg-primary/90">
            ...
            {step === 3 && (
              <div className="animate-in fade-in duration-200">
                ...
                <div className="flex items-center justify-between border-t border-border pt-8">
                  ...
                  <Button onClick={handleCreate} disabled={creating || !name.trim()} className="flex items-center gap-2 rounded-lg bg-primary px-8 py-3 font-semibold text-primary-foreground shadow-lg shadow-primary/25 transition-all hover:scale-[1.02] hover:bg-primary/90 disabled:opacity-50">
        ...
        <p className="mt-8 text-center text-sm text-muted-foreground">
          ...
          <Link to="/docs" className="text-primary hover:underline">{t("createProject.readGuidelines")}</Link>{" "}
